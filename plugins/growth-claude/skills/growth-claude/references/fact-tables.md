...
> **Moved to standalone reference:** See [lifecycle-comms.md](lifecycle-comms.md) for full schema, efficiency rules, and query patterns.
>
> `hs-ai-production.handshake_derived.lifecycle_communication_messages` — ~13B rows. Always filter by `sent_at`. No `profile_id` — join via `user_id` or `email_address`.
